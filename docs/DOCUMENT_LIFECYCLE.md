...
对话记忆保存 user/tool/assistant 事件，并将旧摘要、最近事件和实体关系作为有界非证据上下文，用于理解多轮意图与指代；它不是事实来源。会话或个人文档正文只在用户显式启用时进入当次 corpus，检索出的有限 evidence cards 才会进入 LLM 上下文。未召回页面、原 PDF、密钥和模型隐藏推理都不会进入 prompt。
