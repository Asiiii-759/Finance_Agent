...
配置 LLM 后，系统不再用关键词或“它 / 前者 / 后者”规则生成本轮研究需求。`llm.task_frame` 读取当前请求、对话摘要、最近事件和原子实体回放，输出中文 JSON：目标、实体及来源、最低 evidence requirements、完成标准，或一条澄清问题。
...
实体回放是带 `event_id`、时间、顺序、对象、动作和状态的历史索引，不是实体关系推理器。模型在 TaskFrame 中声明从 `current_request` 或 `conversation_memory` 得到的实体。若多个历史对象都合理，模型必须输出 `clarification_question`，图在不调工具的情况下以 `needs_clarification` 结束。
